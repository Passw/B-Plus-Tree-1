...
public:
  ...
  public:
    ...
    void advance(int distance); // advance iteraton by distance. (call ++(rit)
for advance times)
...
template <class key_type, class val_type> class Node {
public:
  Node() {
    next_leaf = nullptr;
    prev_leaf = nullptr;
...
template <class key_type, class val_type, size_t max_children = 3> class Tree {
...
public:
  ...
  public:
    ...
    // prefix increment operator (++it). it returns a referece to the
    // incremented iterator, avoiding the copy.
    const reverse_iterator &operator++() {
  ...
  void insert(const key_type &key, val_type val) {
    ...
    while (n->keys.size() == max_degree) {
      ...
      /* no matter weather we split the internal node or root node
         We need the "right" node. When we split the nodes that contain records,
         the median was kept. Otherwise, the median was deleted.
      ...
      */
      ...
      if (traverse_indices.size() == 0) {
        ...
        // cout << "size: " << parent->nodes.size() << " " << n->nodes.size() <<
        // " " << right->nodes.size() << endl;

      } else {

        /* when we split the internal node, the original node keeps the half
           capacity as the left node. Also, the median key was added to it's
           parent.
         */
  ...
  void erase(const key_type &key) {
    ...
    /* remember an internal node along with index, whose key is euqal to param
       "key" when we delete the leftmost key in the subtree, we will update the
       internal node's key, which has the same value as param "key". The new key
       will be the new replaced element.
    */
    ...
    */
    while (n->keys.size() < min_keys && n != root) {
      ...
      /* case:2 borrow from left node
         when it's not the leftmost node in the substree and the left node's
         size is big enought. traverse_index minus 1 is because the index of key
         is one less than the index of nodes.
      */
      if (left != nullptr && traverse_index != 0) {
        ...
        if (size > min_keys) {
          ...
          traverse_index--;
          /* if it's leaf nodes, we steal the rightmost key and val in the left
             node and update the parent's key with its key. Otherwise we bring
             down the parent key to the current node and bring up the rightmost
             key in the left node.
          */
          if (records) {
            ...
            n->nodes.insert(n->nodes.begin(),
                            left->nodes[left->nodes.size() - 1]);
            left->nodes.pop_back();
          }
        ...
        /* case3: borrow from right node */
                 ...
                 traverse_index != parent->nodes.size() - 1) {
        ...
        if (size > min_keys) {
          ...
          /* if it's leaf nodes, we steal the leftmost key and val in the right
             node and update the parent's key with its key. Otherwise we bring
             down the parent key to the current node and bring up the leftmost
      ...
      if (left != nullptr && traverse_index != 0) {
        ...
        /* when it's not leaf nodes, bring down the parent key and merge nodes
         * as well */
        if (!records) {
  ...
  void erase(const reverse_iterator &rit) { erase(rit.get_key()); }

  void erase(const iterator &it) { erase(it.get_key()); }

  bool contains(const key_type &k) { return (find(k) != end()); }
